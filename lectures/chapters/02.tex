...
Mějme $G \in \S$, $G = (V,E)$, splňující $d(G) \geq 4k$. Pak $G$ obsahuje podgraf, který je 
\hyperref[k-souvislost]{$k$-souvislý}.
...
G \text{ má } \text{\hyperref[k-souvislost]{$k$-souvislost}.}$
...
\hyperref[k-souvislost]{$k$-souvislý} podgraf.
...
\begin{enumerate}[(a)]
    ...
    $G$ není \hyperref[k-souvislost]{$k$-souvislý}, tj. $X \subseteq V$, $|X|=k-1$ a $X$ je 
    \hyperref[vrchRez]{vrcholový řez}.
\end{enumerate}
...
\begin{figure}[H]
    ...
    \end{tikzpicture}
\end{figure}
$G \setminus X$ je ne\hyperref[souvisly]{souvislý}. Všech je $(k-1) + (k-1) + 1$. $\colorbox{blue!30}{$d_G(x)$} 
\geq 2k - 2$.
...
\begin{enumerate}[(a)]
    ...
    $G + e$ je graf s $V(G)$ a $E(G) \cup \bc{e}$. \\ Je-li $G$ \hyperref[k-souvislost]{2-souvislý}, tak $G+e$ je 
    2-souvislý.
...
\dukaz Každý 2-souvislý graf je \hyperref[souvisly]{souvislý}. Kdyby souvislý neobsahoval kružnici, jedná se o strom. A 
každý strom s alespoň 3 vrcholy má \hyperref[artikulace]{artikulaci}. Protože stromy nemohou být 2-souvislé, a zároveň 
všechny ostatní souvislé grafy obsahují kružnici, i každý 2-souvislý graf obsahuje kružnici.
\hspace{\fill}\qed
...
společné \hyperref[kruznice]{kružnici}.

\dukaz\\
\enquote{$\Leftarrow$}: Předpokládejme, že pro každé $u \not= v$ existuje kružnice $K$, která je obsahuje.

To znamená, že graf je \hyperref[souvisly]{souvislý}. Musíme ještě dokázat, že v něm neexistuje 
\hyperref[artikulace]{artikulace}. Kdyby graf měl artikulaci $v$: 
\begin{figure}[H]
    \centering
    \begin{tikzpicture}[scale=1]
...
\enquote{$\Rightarrow$}: Předpokládejme, že $G$ je \hyperref[k-souvislost]{2-souvislý}. Dokažme indukcí podle 
vzdálenosti $d(u,v)$.
\begin{enumerate} [(a)]
    ...
    \begin{enumerate}[(1)]
        ...
        $x \in C_1$, $x \not= u$, tj. $u$ je artikulace. Což je spor. Takže $G \setminus e$ je souvislý. Tedy existuje 
        cesta $P$ z $u$ do $v$. Pak $P$ je kružnice obsahující $u,v$. 
    \end{enumerate}
    ...
    \hyperref[stc]{cestu} $P$, abychom se dostali z $u$ do $w$, následně se přes $K_1$ dostaneme do $v$. Dále po 
    kružnici do $x_1$, kde si musíme vybrat trasu, která nevede do $w$, tj. směrem do $u$. A tím uzavřeme 
    \textcolor{red}{kružnici} obsahující $u$ a $v$.
\end{enumerate}
\hspace{\fill}\qed
...
\enquote{$\Leftarrow$}: Předpokládejme, že $G \% e$ je \hyperref[k-souvislost]{2-souvislý}, tj. každé 2 vrcholy leží na 
společné kružnici.
\begin{equation}
    x, y \in V(G) \dots \text{existuje } K \text{ v } G \% e \text{ obsahující } x, y
    \begin{cases}
        K \text{ neobsahuje } e_1, e_2 & K \text{ je \hyperref[kruznice]{kružnice} } G. \\
        \\
...
\end{equation}
Máme-li $G_i$, že $G_i \not= G$, tak $G_{i+1}$ je $G_i$, ke kterému přidáme \hyperref[stc]{cestu} $P$ (v $G$), která 
vede mezi 2 vrcholy z $G_i$ a zároveň všechny vrcholy této cesty nejsou v $G_i$.
...
Mějme $G \in \S$, $G = (V,E)$, pak $A \subseteq V(G)$ se nazývá \ii{blok}, jestliže je maximální podmnožina taková, že 
jí indukovaný podgraf je \hyperref[k-souvislost]{2-souvislý}. \\
