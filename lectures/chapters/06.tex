...
Když ke $G$ splňující Chvátalovu podmínku přidáme hranu, která v $G$ není, tak nový graf stále splňuje Chvátalovu podmínku.
...
\begin{align}
    ...
    d(v_{l-1}) &\leq d(x) = k
\end{align}
Tj. máme $k$ vrcholů $v_{l-1}$ s $d(v_{l-1}) \leq k$. Platí $d_k \leq k$, protože máme alespoň $k$ vrcholů stupně $\leq k$. Z Chvátalovy podmínky víme, že $d_{n-k} \geq n-k$, tj. existuje $k+1$ vrcholů $w$ s $d(w) \geq n-k$. Ale $x$ má $d(x) = k$. Tedy existuje $w$ s $d(w) = n-k$, že $\bc{x,w} \not\in E$.
...
\dukaz Jestliže posloupnost nesplňuje \hyperref[chvatal]{Chvátalovu podmínku}, pak existuje číslo $k$ tak, že $k \leq \frac{n}{2}$, $d_k \leq k$ a přitom $d_{n-k} \leq n-k-1$.
...
\dukaz
\begin{itemize}
    \item[$\Rightarrow$:] triviální. % TODO: opravdu?
    \item[$\Leftarrow$:] Nechť $G = (V,E)$ je turnaj, který je silně souvislý. % TODO: dodělat
\end{itemize}
