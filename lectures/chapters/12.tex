...
\begin{figure}[H]
    ...
    \begin{minipage}[c]{0.45\textwidth}
        ...
        \begin{tikzpicture}[
            ...
            \draw[fill=green!10, rounded corners] (-1.5,-1) rectangle (0.5,2);
            \node at (-0.5, 2.3) {$V_1$ (souvislé)};
            
            \draw[fill=green!10, rounded corners] (2.5,-1) rectangle (4.5,2);
            \node at (3.5, 2.3) {$V_2$ (souvislé)};
            ...
            \draw[thick, red] (a) -- (c);
            \draw[thick, red] (b) -- (d);
            \node[red] at (1.5, 0.5) {$S$};
        \end{tikzpicture}

		Cutset \\ $V_1$ i $V_2$ jsou souvislé grafy
    \end{minipage}
    \hfill
    \begin{minipage}[c]{0.45\textwidth}
        \centering
        \begin{tikzpicture}[
            ...
            \draw[fill=gray!10, rounded corners] (-1.5,-1) rectangle (0.5,2);
            \node at (-0.5, 2.3) {$V_1$ (nesouvislé)};
		...
		Obecný řez \\ $V_1$ se rozpadl na více komponent
    \end{minipage}
\end{figure}
...
\end{equation}
Množina $K \oplus C$ leží v prostoru kružnic a je proto součtem hranově disjunktních kružnic. Navíc platí, že $K \oplus C \subseteq T$. To je ale možné pouze pro $K \oplus C = \emptyset$. Takže dostáváme $K = C$, tudíž $K$ je součtem fundamentálních kružnic $K_{c_1}, K_{c_2}, \dots, K_{c_r}$.
\hspace{\fill}\qed
...
\begin{enumerate}[1)]
	...
	\item $B \subseteq E$ je prvek prostoru řezů grafu $G$ právě tehdy, když má sudý počet společných hran s libovolným prvkem prostoru kružnic $W_K$ grafu $G$.
\end{enumerate}
